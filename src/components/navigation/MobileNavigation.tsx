...
export default function MobileNavigation({
  ...
  const bottomNavTabs = mainNavItems.filter((item) =>
          ...
          <div className="fixed left-0 top-0 bottom-0 w-80 bg-white shadow-xl transform transition-transform flex flex-col">
            <div className="p-4 border-b bg-gradient-to-r from-blue-600 to-purple-600 flex-shrink-0">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={onClose}
                  className="p-1 rounded-lg hover:bg-white hover:bg-opacity-20 text-white"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
        ...
        <div className="grid grid-cols-5 gap-1">
          {bottomNavTabs.map((tab) => (
            <NavLink
              ...
              to={tab.path}
              className={({ isActive }) =>
                `flex flex-col items-center justify-end h-14 py-2 px-1 text-xs transition-all ${
                  isActive
