        ...
        {table.rows.map((row: (string | number)[], i: number) => (
          <tr key={i} className="bg-white border-b last:border-b-0">
            {row.map((cell, j: number) => (
              <td
                key={j}
                className={`px-4 py-3 ${j === 0 ? 'font-medium text-gray-900' : ''}`}
              >
                {cell}
              </td>
            ))}
...
export default function ProductDetailPage() {
  ...
  return (
    <div className="space-y-4">
      <Link
        to={`/products/${bankSlug}/${typeSlug}`}
        className="inline-flex items-center text-sm font-medium text-blue-600 hover:underline mb-2"
      >
      ...
      {product.sections.map((section, i) => (
        <InfoSection
          key={i}
          title={section.title}
          icon={iconMap[section.icon] || <FileText className="w-5 h-5" />}
        >
          <div className="space-y-4">
            {section.content.map((contentItem: any, j: number) => {
              if (contentItem.headers) {
                // It's a table
